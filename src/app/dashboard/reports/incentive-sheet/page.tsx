...
export default function IncentiveSheetReportPage() {
  const router = useRouter();
  const { toast } = useToast();
  const [branch, setBranch] = useState('all');
  const [department, setDepartment] = useState('all');
  const [fromDate, setFromDate] = useState<Date | undefined>(new Date(2026, 1, 1));
  const [toDate, setToDate] = useState<Date | undefined>(new Date(2026, 1, 3));
  const [formatType, setFormatType] = useState<'xls'>('xls');

  const handleDownload = () => {
    const reportName = "Incentive Report";

    toast({
      title: 'Downloading Report...',
      description: 'Your incentive report is being generated.',
    });
  ...
  return (
    <div className="flex h-full min-h-screen flex-col bg-background">
      <header className="flex h-16 shrink-0 items-center border-b bg-card px-4">
        ...
        <h1 className="ml-4 text-lg font-semibold">Incentive Report</h1>
      </header>
