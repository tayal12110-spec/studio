...
export default function DailyAttendanceReportPage() {
  ...
  const [date, setDate] = useState<Date | undefined>(new Date(2026, 1, 1));
  ...
  const handleDownload = () => {
    ...
    toast({
      ...
      description: 'Your daily attendance report is being generated.',
    });
    
    const dateString = date ? format(date, 'dd MMM yyyy') : 'Date N/A';
    ...
    const queryParams = new URLSearchParams({
        ...
        month: dateString, // Re-using month param for date
        branch: branchString,
        format: formatType,
    });
  ...
  return (
    <div className="flex h-full min-h-screen flex-col bg-background">
      <header className="flex h-16 shrink-0 items-center border-b bg-card px-4">
        ...
        <h1 className="ml-4 text-lg font-semibold">Daily Attendance Report</h1>
      </header>
          ...
          <Popover>
            <PopoverTrigger asChild>
              ...
              >
                ...
                {date ? format(date, 'd MMM yyyy') : <span>Select date</span>}
              </Button>
            </PopoverTrigger>
