...
import { ArrowLeft, Calendar as CalendarIcon, File as FileIcon } from 'lucide-react';
...
export default function CompanyRosterReportPage() {
  const router = useRouter();
  const { toast } = useToast();
  const [branch, setBranch] = useState('all');
  const [department, setDepartment] = useState('all');
  const [fromDate, setFromDate] = useState<Date | undefined>(new Date(2026, 1, 1));
  const [toDate, setToDate] = useState<Date | undefined>(new Date(2026, 1, 1));
  const [formatType, setFormatType] = useState<'pdf'>('pdf');

  const handleDownload = () => {
    const reportName = "Company Roster Report";

    toast({
      title: 'Downloading Report...',
      description: 'Your company roster report is being generated.',
    });
  ...
  return (
    <div className="flex h-full min-h-screen flex-col bg-background">
      <header className="flex h-16 shrink-0 items-center border-b bg-card px-4">
        ...
        <h1 className="ml-4 text-lg font-semibold">Company Roster Report</h1>
      </header>
      ...
      <main className="flex-1 space-y-6 p-6">
        ...
        <div>
            ...
            <RadioGroup value={formatType} onValueChange={(value) => setFormatType(value as 'pdf')} className="mt-2">
                <div className="rounded-lg border has-[:checked]:border-primary has-[:checked]:bg-primary/10">
                <Label htmlFor="format-pdf" className="flex cursor-pointer items-center gap-3 p-3">
