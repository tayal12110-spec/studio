...
import { ArrowLeft, Calendar as CalendarIcon, FileSpreadsheet, File as FileIcon } from 'lucide-react';
...
export default function DetailedAttendanceReportPage() {
  ...
  return (
    <div className="flex h-full min-h-screen flex-col bg-background">
      <header className="flex h-16 shrink-0 items-center border-b bg-card px-4">
        ...
        <h1 className="ml-4 text-lg font-semibold">Detailed Attendance Report</h1>
      </header>
      ...
      <main className="flex-1 space-y-6 p-6">
        ...
        <div className="flex items-center justify-between rounded-lg border p-4">
            <div className="flex items-center gap-2">
                ...
                <Label htmlFor="payroll-cycle-switch" className="font-medium">Payroll Attendance Cycle</Label>
            </div>
            <Switch
                id="payroll-cycle-switch"
                checked={usePayrollCycle}
                onCheckedChange={setUsePayrollCycle}
            />
        </div>
            ...
            <RadioGroup value={formatType} onValueChange={(value) => setFormatType(value as 'pdf' | 'xls')} className="mt-2 grid grid-cols-2 gap-4">
                <Label
                    ...
                    className={cn(
                        ...
                        formatType === 'pdf' ? 'border-primary bg-primary/10' : 'border-input'
                    )}
                >
                ...
                <Label
                    ...
                    className={cn(
                        "flex cursor-pointer items-center justify-center gap-2 rounded-lg border-2 p-3",
                        formatType === 'xls' ? 'border-primary bg-primary/10' : 'border-input'
                    )}
                >
                    <RadioGroupItem value="xls" id="format-xls" className="sr-only" />
